...
# Masterjem was modified from Jacob Mattingley's original jemdoc.
#
# This file is part of jemdoc. 
#
# jemdoc is a free software; you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation; either version 3 of the License, or (at your option) any later version.
#
...
def procfolder(i,o):
  ...
  if "conferences.bib" in os.listdir(i) and "journals.bib" in os.listdir(i):
    combinefiles(i,o)
  for f in os.listdir(i):
    currentfile = os.path.basename(f)
    if "conferences.bib" in currentfile:
      processbib(i+"/"+currentfile)
    elif "journals.bib" in currentfile:
      processbib(i+"/"+currentfile)
    elif "publications.bib" in currentfile:
      processbib(i+"/"+currentfile)

  for file in os.listdir(i):
    currentfile = os.path.basename(file)
    if ".jemdoc" in currentfile:
      confnames = []
      conf = parseconf(confnames)
      thisout = o+"/"+ re.sub(r'.jemdoc$', '', currentfile) + '.html'
      infile = open(i+"/"+currentfile, 'rUb')
      outfile = open(thisout, 'w')

      f = controlstruct(infile, outfile, conf, currentfile)
      procfile(f)
...
def copy(i,o):
  ...
  for file in os.listdir(i):
    if os.path.isdir(file):
      ...
      copyfile(i+"/"+file,o+"/"+file)

def procbibentry(f):
  prevyear = "8723467868235"
  headerprinted = False
  journal_header_printed = False
  conf_header_printed = False
  ip_header_printed = False
 
  if "publications.bib" not in f.inname: 
      journal_header_printed = True
      conf_header_printed = True
      ip_header_printed = True
  ...
  for l in f.inf:
    ...
    if l.startswith("@"):
      ...
      month = None
      note = None
      if l.startswith("@article") and not journal_header_printed:
        f.outf.write("== Journals \n")
        journal_header_printed = True
      elif l.startswith("@inproceedings") and not conf_header_printed:
        f.outf.write("== Conferences \n")
        conf_header_printed = True
      elif l.startswith("@unpublished") and not ip_header_printed:
        f.outf.write("== In Progress \n")
        ip_header_printed = True
    elif l is not "}":
      if "=" in l:
      ...
      if "thesis.bib" in f.inname and headerprinted is False:
        headerprinted = True
        hb(f.outf,f.conf['thesisheading'],"Thesis")
      elif "thesis.bib" not in f.inname and yr != prevyear:
        prevyear = yr
      ...
      if btitle is not None:
        if "in preparation" not in btitle and "submitted to" not in btitle:
...
def parsebibconf(cns):
  syntax = {}
  warn = False # jem. make configurable?
  # manually add the defaults as a file handle.
  fs = [StringIO.StringIO(bibconf())]
  for sname in cns:
    fs.append(open(sname, 'rb'))
...
def processbib(i):
  confnames = []
  conf = parsebibconf(confnames)
  o = re.sub(r'.bib$', '', i) + '.jemdoc'
  infile = open(i, 'rUb')
  outfile = open(o, 'w')
  f = controlstruct(infile, outfile, conf, i)

  outfile.write("# jemdoc: menu{MENU}{publications.html}")
  outfile.write("\n")
  outfile.write("# jemdoc: addcss{jemdoc_custom.css}")
  outfile.write("\n")
  outfile.write("= Publications \n \n")
  if "conferences" in i:
    outfile.write("== Conferences")
  elif "journals" in i:
  ...
  if "publications" in i:
    ...
    for file in os.listdir(d):
      if "thesis.bib" in file:
        thesisfile = open(d+"/thesis.bib",'rUb')
        i2 = d+"/thesis.bib"
        f2 = controlstruct(thesisfile, outfile, conf, i2)
        procbibentry(f2)
        found = True
      elif found is True:
        break
...
def combinefiles(i,o):
  if os.path.isfile("publications.bib"):
    os.remove("publications.bib")

  outfile = open(i+"/"+"publications.bib", 'w')
  injournals = open(i+"/"+"journals.bib", 'r')
  inconferences = open(i+"/"+"conferences.bib", 'r')

  copylines(injournals,outfile)
  copylines(inconferences,outfile)
  outfile.close()
  injournals.close()
  inconferences.close()
...
def main():
  ...
  for i in range(1, len(sys.argv), 2):
    if sys.argv[i] == '-o':
      ...
      if cssoverride:
        raise RuntimeError("only one CSS file please")
      else:
        cssoverride = True
        customcss = sys.argv[i+1]
    elif sys.argv[i] == '-i':
      extrafiles.append(sys.argv[i+1])
  ...
  if outname is None:
    thisout = inname+"_HTML_files"
  else:
    thisout = outname

  if not os.path.isdir(thisout):
    os.mkdir(thisout)
  else:
    rmtree(thisout)
    os.mkdir(thisout)
  ...
  if cssoverride:
    copyfile(inname+"/"+customcss,thisout+"/"+customcss)

  if extrafiles:
    for f in extrafiles:
      copy(inname+"/"+f,thisout+"/"+f)

  copiedmenu = False
  if not os.path.exists("MENU"):
    copyfile(inname+"/MENU",os.getcwd()+"/MENU")
    copiedmenu = True

  procfolder(inname,thisout)

  if copiedmenu:
    os.remove(os.getcwd()+"/MENU")
