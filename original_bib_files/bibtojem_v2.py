...
def procfile(f):
	...
	for l in f.inf:
		...
		papers_dict[paper_idx]["title"] = title
		papers_dict[paper_idx]["authors"] = authorstring
		papers_dict[paper_idx]["btitle"] = btitle	
		papers_dict[paper_idx]["pgs"] = pgs
		papers_dict[paper_idx]["yr"] = yr
		papers_dict[paper_idx]["org"] = org
		papers_dict[paper_idx]["doi"] = doi 
		papers_dict[paper_idx]["vol"] = vol 
		papers_dict[paper_idx]["school"] = school 
		papers_dict[paper_idx]["address"] = address
		papers_dict[paper_idx]["num"] = num 
		papers_dict[paper_idx]["month"] = month
		papers_dict[paper_idx]["note"] = note  
		
		paper_idx += 1	
		max_year = max(max_year,yr)

	year = max_year
	for paper_idx in papers_dict:
			if yr != prevyear:
				prevyear = yr
...
def main():
	...
	if os.path.isfile("thesis.bib"):
		thesisfile = open("thesis.bib",'rUb')
